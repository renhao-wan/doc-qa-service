...
服务通过 Cloudflare **橙云代理**暴露，源站只接受 CF 回源。

### 7.1 为什么用 `wanrenhao.xyz` 而不是 `wanrenhao.me`

`.me` 后缀**不在工信部批复的顶级域名列表里**，因此在大陆**根本无法备案**。而备案是「域名解析到大陆服务器」的前提——没有备案，域名一旦回源到这台阿里云服务器，就会命中 **ICP 拦截**（识别方法见 §7.5）。

这不是配置问题：`Flexible`、`Full (strict)`、Origin Rules 挨个试一遍都绕不过去，因为拦截发生在链路上，不在 CF 与源站之间。

⚠️ **反例会把人带偏**：当时 `aisketch.wanrenhao.me` 一直能正常访问，看起来「这个域名没问题」。但那个子域**不回源到这台服务器**（内容托管在 CF 边缘）。判断依据只能是**它有没有回源到大陆服务器**，而不是「同一个域名下别的子域能不能用」。

### 7.2 Cloudflare 三件套

| 设置 | 值 | 不这么设的后果 |
|---|---|---|
| 代理状态 | **橙云 Proxied** | 灰云下浏览器直连源站，拿到源站那张 SAN 不覆盖当前域名的证书 → **证书错误**；源站 IP 也会暴露 |
| SSL/TLS 模式 | **Full** | `Full (strict)` 会校验源站证书是否覆盖请求的域名，不覆盖时握手失败（CF 报 **525**）；`Flexible` 则走 80 端口明文回源 |
| Always Use HTTPS | 开 | 否则 80 端口明文可访问 |

### 7.3 源站证书

`/opt/doc-qa-service/certs/` 里的 `origin.crt` / `origin.key` 是 Cloudflare 后台签发的 **Origin Certificate**：

- 只能用于 CF 回源，**不被浏览器信任**——这没关系，浏览器看到的是 CF 的边缘证书
- 有效期很长（本项目这张到 2041 年）
- **SAN 在签发时就固定了**，之后换域名不会自动扩展

⚠️ **SAN 决定了 `Full (strict)` 能不能用**：CF 免费版改不了回源时的 Host / SNI（Origin Rules 的这两项是 Enterprise 专属），所以**证书 SAN 必须覆盖实际使用的域名**。本项目当前用 `Full` 而非 `Full (strict)`，正是因为源站证书是为 `.me` 签发的，SAN 不覆盖 `.xyz`。

⚠️ 本地 `certs/` 那份是**自签证书**，与服务器上这份用途不同，不要混用。

### 7.4 换域名 / 换服务器要重做哪些

| 事项 | 在哪做 |
|---|---|
| 新域名加 A 记录指向源站 IP | CF DNS |
| 按新域名重新签发 Origin Certificate（只有想用 `Full (strict)` 才需要） | CF → SSL/TLS → Origin Server |
| 修改 NS 指向 CF | 域名注册商 |
...
```bash
nslookup -type=NS <你的域名> a.nic.xyz
```

RDAP 显示新 NS 只说明**注册商已提交**，不代表解析已切换——两者会打架一阵子，这是正常的中转状态。

### 7.5 判断问题出在 CF 还是源站

不要在 CF 后台反复改配置试。**从外部**带不同的 Host 直连源站 IP 做对照，一次就能把两者分开：
...
| 返回 | 结论 |
|---|---|
| 你的页面 | 源站与链路都正常，问题在 CF 配置（SSL 模式、代理状态） |
| `403` + `Server: Beaver` + 备案页 | **链路上有 ICP 拦截**，与 CF 无关 |
| 连接被重置 | 443 的 SNI 被阻断 |

⚠️ **这个对照必须在外网做**。在服务器上 `curl` 自己的公网 IP 走的是内网出口，**不经过拦截设备**，结果永远是 200——拿它下结论会完全跑偏。
...
⚠️ **判读那条校验的结果时，先看状态码**（它是带重试的，30×2s，与 web 健康检查一致）：

| 状态码 | 含义 | 处置 |
|---|---|---|
| `000` | **连不上**——多半只是容器还没监听 | 脚本会重试到 60 秒；仍失败才是真没起来 |
| `403` | 端口通了、SearXNG 明确拒绝 | 这才是 `settings.yml` 没就位，重试无意义 |

重试是必需的：脚本在几步之前刚 `restart searxng`，而 SearXNG 从进程启动到监听端口
有一段空窗。2026-09-14 这条校验第一次上线时正是撞上空窗——拿到 `000` 而失败，
提示文案却按 403 的方向说「多半是 settings.yml 没就位」，把一次**误报**指成了错误的排查方向。
